...
void treasurehunt::newstartgame(name username)
{
    // Ensure this action is authorized by the player
    require_auth(username);
    // Create a record in the table if the player doesn't exist in our app yet
...
void treasurehunt::destination(name username, uint8_t selected_map_player)
{
    ...
    _users.modify(user, username, [&](auto &modified_user) {
        modified_user.game_data.selected_map_player = selected_map_player;
    });
}
...
void treasurehunt::playerticket(name username, uint16_t ticket_player)
{
    ...
    _users.modify(user, username, [&](auto &modified_user) {
        game game_data;
        modified_user.game_data.ticket_player = ticket_player;
    });
}
